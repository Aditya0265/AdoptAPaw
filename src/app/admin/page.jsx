  ...
  const handleStatusChange = async (id, newStatus, visitDate = null) => {
    try {
      const response = await fetch(`/api/admin/applications/${id}`, {
        ...
        body: JSON.stringify({
          status: newStatus,
          ...(newStatus === 'HOME_VISIT_SCHEDULED' && { homeVisitDate: visitDate }),
          ...(newStatus === 'FINAL_VISIT_SCHEDULED' && { finalVisitDate: visitDate }),
        }),
      });
      ...
      <main className="flex-grow pt-24 pb-20">
        <div className="container-custom">
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden mb-8">
            <div className="p-6">
              <h2 className="text-xl font-bold text-gray-800 mb-4">Adoption Applications</h2>
                ...
                <div className="md:w-64">
                  <div className="flex items-center h-full">
                    ...
                    >
                      ...
                      <option value="FINAL_VISIT_SCHEDULED">Final Visit Scheduled</option>
                    ...
                    {filteredApplications.length === 0 ? (
                      ...
                      filteredApplications.map((application) => (
                        <tr key={application.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900">{application.dog.name}</div>
                            <div className="text-sm text-gray-500">{application.dog.breed}</div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div className="flex space-x-2">
                              ...
                              {application.status === 'SUBMITTED' && (
                                <button
                                  onClick={() => handleScheduleHomeVisit(application)}
                              ...
                              {application.status === 'HOME_VISIT_SCHEDULED' && (
                                <button
                                  onClick={() => handleCompleteHomeVisit(application.id)}
                              ...
                              {application.status === 'HOME_VISIT_COMPLETED' && (
                                <button
                                  onClick={() => handleScheduleFinalVisit(application)}
                              ...
                              {application.status === 'FINAL_VISIT_SCHEDULED' && (
                                <button
                                  onClick={() => handleCompleteAdoption(application.id)}
                              ...
                              {['SUBMITTED', 'HOME_VISIT_SCHEDULED', 'HOME_VISIT_COMPLETED', 'FINAL_VISIT_SCHEDULED'].includes(application.status) && (
                                <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl shadow-md overflow-hidden">
              <div className="p-6">
                <h2 className="text-xl font-bold text-gray-800 mb-4">Scheduled Home Visits</h2>
                ...
                <ul className="divide-y divide-gray-200">
                      ...
                      <li key={`home-${app.id}`} className="py-4">
                        <div className="flex justify-between">
                          <div>
                            ...
                            <p className="text-sm text-gray-500">{app.dog.name} ({app.dog.breed})</p>
                          </div>
                          <div className="text-right">
                            <p className="font-medium">
                              {new Date(app.homeVisitDate).toLocaleDateString('en-IN')}
                            </p>
                            <p className="text-sm text-gray-500">
                              {new Date(app.homeVisitDate).toLocaleTimeString('en-IN', {
                                hour: '2-digit',
                                minute: '2-digit'
                              })}
                            </p>
                        ...
                        <div className="mt-2">
                          <div className="flex items-start">
                            ...
                            <p className="text-sm text-gray-600">{app.user.address}</p>
                          </div>
                  ...
                  {filteredApplications.filter(app => app.status === 'HOME_VISIT_SCHEDULED').length === 0 && (
            ...
            <div className="bg-white rounded-xl shadow-md overflow-hidden">
              <div className="p-6">
                <h2 className="text-xl font-bold text-gray-800 mb-4">Scheduled Final Visits</h2>
                ...
                <ul className="divide-y divide-gray-200">
                    ...
                    .map(app => (
                      <li key={`final-${app.id}`} className="py-4">
                        <div className="flex justify-between">
                          ...
                          <div className="text-right">
                            <p className="font-medium">
                              {new Date(app.finalVisitDate).toLocaleDateString('en-IN')}
                            </p>
                            <p className="text-sm text-gray-500">
                              {new Date(app.finalVisitDate).toLocaleTimeString('en-IN', {
                                hour: '2-digit',
                                minute: '2-digit'
                              })}
                            </p>
                  ...
                  {filteredApplications.filter(app => app.status === 'FINAL_VISIT_SCHEDULED').length === 0 && (
                    <li className="py-4 text-center text-gray-500">
          ...
          >
            <div className="p-6">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg">
                        <p><span className="font-medium">Name:</span> {selectedApplication.dog.name}</p>
                        <p><span className="font-medium">Breed:</span> {selectedApplication.dog.breed}</p>
                        <p><span className="font-medium">Age:</span> {selectedApplication.dog.age}</p>
                        <p><span className="font-medium">Gender:</span> {selectedApplication.dog.gender === 'MALE' ? 'Male' : 'Female'}</p>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <p><span className="font-medium">Address:</span> {selectedApplication.user.address}</p>
                        <p className="mt-2">
                    ...
                    <div>
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <p><span className="font-medium">Applied On:</span> {new Date(selectedApplication.createdAt).toLocaleString('en-IN')}</p>
                        
                        {selectedApplication.homeVisitDate && (
                          <p>
                            <span className="font-medium">Home Visit Scheduled For:</span> {' '}
                            {new Date(selectedApplication.homeVisitDate).toLocaleString('en-IN')}
                          </p>
                        )}
                        
                        {selectedApplication.finalVisitDate && (
                          <p>
                            <span className="font-medium">Final Visit Scheduled For:</span> {' '}
                            {new Date(selectedApplication.finalVisitDate).toLocaleString('en-IN')}
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {selectedApplication.status === 'HOME_VISIT_SCHEDULED' && (
                        <button
                          onClick={() => {
                            closeModal();
                      ...
                      {selectedApplication.status === 'HOME_VISIT_COMPLETED' && (
                        <button
                          onClick={() => {
                            closeModal();
                      ...
                      {selectedApplication.status === 'FINAL_VISIT_SCHEDULED' && (
                        <button
                          onClick={() => {
                            closeModal();
                      ...
                      {['SUBMITTED', 'HOME_VISIT_SCHEDULED', 'HOME_VISIT_COMPLETED', 'FINAL_VISIT_SCHEDULED'].includes(selectedApplication.status) && (
                        <button
                          onClick={() => {
                            closeModal();
              ...
              {modalType === 'homeVisit' && (
                <>
                  <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold text-gray-800">Schedule Home Visit</h2>
                    <button onClick={closeModal} className="text-gray-500 hover:text-gray-700">
                      <FiX className="h-6 w-6" />
                    </button>
                  </div>
                  ...
                  <form onSubmit={handleModalSubmit} className="space-y-6">
                    <div>
                      <p className="mb-2">
                        <span className="font-medium">Applicant:</span> {selectedApplication.user.name}
                      </p>
                      <p className="mb-2">
                        <span className="font-medium">Dog:</span> {selectedApplication.dog.name} ({selectedApplication.dog.breed})
                      </p>
                      <p className="mb-4">
                        <span className="font-medium">Address:</span> {selectedApplication.user.address}
                      </p>
                    </div>
                    
                    <div>
                      <label htmlFor="homeVisitDate" className="input-label">Home Visit Date & Time</label>
                      <input
                        type="datetime-local"
                        id="homeVisitDate"
                        name="homeVisitDate"
              ...
              {modalType === 'finalVisit' && (
                <>
                  <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold text-gray-800">Schedule Final Visit</h2>
                    <button onClick={closeModal} className="text-gray-500 hover:text-gray-700">
                      <FiX className="h-6 w-6" />
                    </button>
                  </div>
                  ...
                  <form onSubmit={handleModalSubmit} className="space-y-6">
                    ...
                    <div>
                      <label htmlFor="finalVisitDate" className="input-label">Final Visit Date & Time</label>
                      <input
                        type="datetime-local"
                        id="finalVisitDate"
                        name="finalVisitDate"
